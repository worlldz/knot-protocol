...
async function settleHandler(request: NextRequest): Promise<NextResponse<unknown>> {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Invalid settlement request" }, { status: 400 });

  return NextResponse.json({
    provider: "Arc Sentinel",
    service: "Live Arc wallet risk evidence",
    ...parsed.data,
...
export const POST = sellerAddress && isAddress(sellerAddress)
  ? withX402(settleHandler, {
      accepts: { scheme: "exact", payTo: sellerAddress, price: "$0.024", network: ARC_TESTNET_CAIP },
      description: "Settle verified Arc Sentinel wallet evidence",
      mimeType: "application/json",
      serviceName: "Arc Sentinel via KNOT",
      tags: ["wallet-risk", "live-data", "agents", "Arc", "USDC"],
